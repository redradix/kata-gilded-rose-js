...
describe("GildedRose shop manager", function () {
    ...
    it("decreases by 1 the quality and remaining sellIn days of regular items", function () {
        ...
        expected.forEach(function (testCase, idx) {
            expect(items[idx].quality).to.deep.equal(testCase.quality)
            expect(items[idx].sellIn).to.deep.equal(testCase.sellIn)
        })
    })
    ...
    it("does not alter the quality of 'Sulfuras', wich is always 80", function () {
        ...
        expect(items[0].quality).to.deep.equal(80)
        expect(items[0].sellIn).to.deep.equal(0)
    })
    ...
    it("does not increase quality over 50", function () {
        ...
        expect(items[0].quality).to.deep.equal(50)
        expect(items[0].sellIn).to.deep.equal(3)
    })
})
